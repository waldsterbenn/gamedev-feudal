...
### Agricultural Suppression & Ecosystem Interaction
High tree coverage chokes out fields. Conversely, clear-cutting a forest strips away the shade required by sensitive wild resources. 

**Effective crop yield is calculated as:**
$$Fertility_{effective} = Fertility_{base} \times (1.0 - CanopyDensity)$$

> ⚠️ **Ecosystem Note:** Wild foraging yields (like mushrooms) scale down linearly as canopy protection is removed.
...
*   **Work Camp:** The mandatory first step of colonization. A transient, survival-focused outpost for extracting raw resources consisting of primitive structures (tents, dugout huts, charcoal woodstacks). Functions as live-in workplaces for laborers operating far from towns. *Starts completely bare with no starter resources or free jobs.*
...
1.  **The Population Pipeline:** Populants do not materialize automatically out of thin air. The player must find NPCs wandering the open world, actively recruit them to their faction, and then manually assign them to reside at a controlled node.
2.  **Residential Lock:** When a Populant is assigned to a node, that node becomes their primary residence. They consume food from its inventory and use its local housing (tents/hovels).
3.  **The Commuting Rule:** A Populant assigned to Node A can fulfill a job slot on Node A **OR** any directly connected neighboring node on the world map graph. This allows a central Village node to act as a residential hub while its workers commute out to a neighboring mining or logging Camp.
...
| Job | Primary Yield | Environmental Modifier |
| :--- | :--- | :--- |
| **Forager** | Berries, Mushrooms | Mushroom production drops heavily if the surrounding `canopy_density` falls, reflecting a loss of shaded, damp soil. |
| **Woodcutter** | Timber | Directly degrades the node's `canopy_density` with every unit gathered, leading to structural deforestation and diminishing yield returns over time. |

### 6.2 Localized Inventory & Logistics
