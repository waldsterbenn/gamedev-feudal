...
func _load_map(dir: String, map_type: int, index: int, resource_loader_cache_mode : int) -> bool:
	...
	# Runtime fallback: if ResourceLoader couldn't load the texture
	# (e.g. terrain data directory was excluded from import via .gdignore),
	# load the image directly from disk.
	if tex == null:
		var global_fpath := ProjectSettings.globalize_path(fpath)
		if _channel_can_be_saved_as_png(map_type):
			if FileAccess.file_exists(global_fpath):
				_logger.debug(str("ResourceLoader failed for ", fpath,
					", falling back to Image.load_from_file at runtime"))
				map.image = Image.load_from_file(global_fpath)
				if map.image != null:
					map.image = _ensure_map_format(map.image, map_type, index)
					tex = ImageTexture.create_from_image(map.image)
					must_load_image_in_editor = false
		else:
			# .res files should be loadable by ResourceLoader, but as a last
			# resort try loading via the ResourceLoader with a direct path
			_logger.warn(str("ResourceLoader failed for resource file ", fpath))
	...
	if map_type == CHANNEL_HEIGHT and map.image != null:
